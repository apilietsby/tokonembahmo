...
const noAdmin = '6285700800278'; // NO WA ADMIN
const db = supabase.createClient(supabaseUrl, supabaseKey);

let products = [];
let cart = [];
let currentProduct = null; // Produk yg sedang dibuka di popup
let currentQty = 1;
...
window.openDetail = (id) => {
    ...
    currentQty = 1; // Reset jumlah jadi 1
    ...
    container.innerHTML = cart.map((item, idx) => {
        ...
        <div style="display:flex; gap:10px; margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:10px;">
            ...
            <div style="flex:1;">
                ...
                <div style="color:var(--primary);">Rp ${Number(item.price).toLocaleString()}</div>
                <div style="font-size:13px; color:#666;">Jumlah: ${item.qty}</div>
            </div>
            <button onclick="hapusItem(${idx})" style="background:red; color:white; border:none; padding:5px 10px; border-radius:5px;">Hapus</button>
        </div>`;
    }).join('');
...
window.checkoutWA = async () => {
    ...
    if(!name || !phone || !loc || !addr) return alert("Lengkapi data pengiriman!");
    ...
    btn.innerText = "Beli via WhatsApp"; btn.disabled = false;
};
...
window.mitraMode = (mode) => {
    ...
    // Ubah warna tombol
    document.getElementById('btn-daftar').style.background = mode === 'daftar' ? '#0f5132' : '#ccc';
    document.getElementById('btn-daftar').style.color = mode === 'daftar' ? 'white' : '#333';
    document.getElementById('btn-login').style.background = mode === 'login' ? '#0f5132' : '#ccc';
...
window.daftarMitra = async (e) => {
    ...
    else { alert("Berhasil! Tunggu Admin."); e.target.reset(); }
};
...
window.cekMitra = async () => {
    ...
    if(!data) {
        resDiv.innerText = "Tidak ditemukan."; resDiv.style.color = "red";
    } else if (!data.approved) {
        resDiv.innerText = "Masih menunggu persetujuan."; resDiv.style.color = "orange";
    } else {
        const link = window.location.origin + window.location.pathname + "?ref=" + data.referral_code;
        resDiv.innerHTML = `✅ Aktif!<br>Link: <input value="${link}" readonly style="width:100%">`;
